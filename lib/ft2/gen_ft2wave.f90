subroutine gen_ft2wave(itone,nsym,nsps,fsample,f0,cwave,wave,icmplx,nwave)
  ...
  real wave(nwave)
  complex cwave(nwave)
  real, save :: pulse(3456)     !288*4*3  (must persist between calls)
  real dphi(0:250000-1)
  integer itone(nsym)
  integer, save :: last_nsps = -1
  real, save :: last_fsample = -1.0

  twopi=8.0*atan(1.0)
  dt=1.0/fsample
  hmod=1.0

! Recompute the smoothed frequency-deviation pulse when nsps changes
  if(nsps.ne.last_nsps .or. fsample.ne.last_fsample) then
     do i=1,3*nsps
